...
export const generateWorkoutAdvice = async (userQuery: string): Promise<string> => {
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: userQuery,
      config: {
        systemInstruction: `You are 'Coach L7', an elite performance trainer for Level 7 Fitness in Nanded, Maharashtra.
        Your gym is located at Raj Mall, Anand Nagar Chowk.
        Your tone is motivating, energetic, and professional.
        If asked about location, always refer to Raj Mall, Anand Nagar Chowk.
